...
from stripe_sub5.six.moves.urllib.parse import parse_qsl, quote_plus
...
def _console_log_level():
    if stripe_sub5.log in ["debug", "info"]:
        return stripe_sub5.log
...
def _test_or_live_environment():
    if stripe_sub5.api_key is None:
        return
    match = re.match(r"sk_(live|test)_", stripe_sub5.api_key)
    if match is None:
        return
...
def get_object_classes():
    ...
    from stripe_sub5.object_classes import OBJECT_CLASSES
...
def convert_to_stripe_object(
    ...
    if isinstance(resp, stripe_sub5.stripe_response.StripeResponse):
    ...
    if isinstance(resp, list):
        ...
        resp, stripe_sub5.stripe_object.StripeObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, six.string_types):
            klass = get_object_classes().get(
                klass_name, stripe_sub5.stripe_object.StripeObject
            )
        else:
            klass = stripe_sub5.stripe_object.StripeObject
