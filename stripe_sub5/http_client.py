...
import stripe_sub5
from stripe_sub5 import error, util, six
from stripe_sub5.request_metrics import RequestMetrics
...
try:
    from stripe_sub5.six.moves import urllib
except ImportError:
...
from stripe_sub5.six.moves.urllib.parse import urlparse
...
class HTTPClient(object):
    ...
    def _max_network_retries(self):
        from stripe_sub5 import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if stripe_sub5.enable_telemetry and last_request_metrics:
            telemetry = {
    ...
    def _record_request_metrics(self, response, request_start):
        _, _, rheaders = response
        if "Request-Id" in rheaders and stripe_sub5.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = stripe_sub5.ca_bundle_path
        else:
            kwargs["verify"] = False
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.CAINFO, stripe_sub5.ca_bundle_path)
        else:
