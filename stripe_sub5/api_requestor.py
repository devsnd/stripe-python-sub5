...
import stripe_sub5
from stripe_sub5 import error, oauth_error, http_client, version, util, six
from stripe_sub5.multipart_data_generator import MultipartDataGenerator
from stripe_sub5.six.moves.urllib.parse import urlencode, urlsplit, urlunsplit
...
class APIRequestor(object):
    def __init__(
        ...
        self.api_base = api_base or stripe_sub5.api_base
        self.api_key = key
        self.api_version = api_version or stripe_sub5.api_version
        self.stripe_account = account

        self._default_proxy = None

        from stripe_sub5 import verify_ssl_certs as verify
        from stripe_sub5 import proxy

        if client:
            self._client = client
        elif stripe_sub5.default_http_client:
            self._client = stripe_sub5.default_http_client
            ...
            stripe_sub5.default_http_client = http_client.new_default_http_client(
    ...
    def request_headers(self, api_key, method):
        ...
        if stripe_sub5.app_info:
            user_agent += " " + self.format_app_info(stripe_sub5.app_info)
        ...
        for attr, func in [
            ...
            ua[attr] = val
        if stripe_sub5.app_info:
            ua["application"] = stripe_sub5.app_info
    ...
    def request_raw(
        ...
        if self.api_key:
            ...
            from stripe_sub5 import api_key
