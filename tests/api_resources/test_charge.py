...
class TestCharge(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.Charge.list()
        request_mock.assert_requested("get", "/v1/charges")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.Charge)

    def test_is_searchable(self, request_mock):
        resources = stripe_sub5.Charge.search(query='currency:"USD"')
        ...
        for c in resources.auto_paging_iter():
            assert isinstance(c, stripe_sub5.Charge)
            cnt += 1

        assert cnt == 1

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.Charge.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/charges/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Charge)

    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.Charge.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.Charge.modify(
    ...
    def test_can_capture(self, request_mock):
        charge = stripe_sub5.Charge.retrieve(TEST_RESOURCE_ID)
    ...
    def test_can_capture_classmethod(self, request_mock):
        resource = stripe_sub5.Charge.capture(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/charges/%s/capture" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Charge)
