...
class TestSKU(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.SKU.list()
        request_mock.assert_requested("get", "/v1/skus")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.SKU)

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.SKU.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested("get", "/v1/skus/%s" % TEST_RESOURCE_ID)
        assert isinstance(resource, stripe_sub5.SKU)

    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.SKU.create(
            currency="usd",
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.SKU.modify(
    ...
    def test_can_delete(self, request_mock):
        resource = stripe_sub5.SKU.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/skus/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
