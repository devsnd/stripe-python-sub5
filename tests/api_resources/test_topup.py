...
class TestTopup(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.Topup.list()
        request_mock.assert_requested("get", "/v1/topups")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.Topup)

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.Topup.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/topups/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Topup)

    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.Topup.create(
            amount=100,
            currency="usd",
            source="src_123",
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.Topup.modify(
    ...
    def test_can_cancel_classmethod(self, request_mock):
        resource = stripe_sub5.Topup.cancel(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/topups/%s/cancel" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Topup)
