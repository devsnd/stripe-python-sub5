...
class TestLocation(object):
    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.terminal.Location.create(
            display_name="name",
            address={
                "line1": "line1",
                "country": "US",
        ...
        assert isinstance(resource, stripe_sub5.terminal.Location)

    def test_is_listable(self, request_mock):
        resources = stripe_sub5.terminal.Location.list()
        request_mock.assert_requested("get", "/v1/terminal/locations")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.terminal.Location)

    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.terminal.Location.modify(
    ...
    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.terminal.Location.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    def test_can_delete(self, request_mock):
        resource = stripe_sub5.terminal.Location.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/terminal/locations/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
