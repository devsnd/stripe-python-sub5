...
class TestReader(object):
    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.terminal.Reader.create(
            registration_code="a-b-c", label="name"
        )
        request_mock.assert_requested("post", "/v1/terminal/readers")
        assert isinstance(resource, stripe_sub5.terminal.Reader)

    def test_is_listable(self, request_mock):
        resources = stripe_sub5.terminal.Reader.list()
        request_mock.assert_requested("get", "/v1/terminal/readers")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.terminal.Reader)

    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.terminal.Reader.modify(
    ...
    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.terminal.Reader.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    def test_can_delete(self, request_mock):
        resource = stripe_sub5.terminal.Reader.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/terminal/readers/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
    ...
    def test_can_present_payment_method(self, request_mock):
        resource = stripe_sub5.terminal.Reader.TestHelpers.present_payment_method(
            TEST_RESOURCE_ID
        )
        request_mock.assert_requested(
            "post",
