...
class TestPaymentIntent(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.PaymentIntent.list()
        request_mock.assert_requested("get", "/v1/payment_intents")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.PaymentIntent)

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.PaymentIntent.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/payment_intents/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.PaymentIntent)

    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.PaymentIntent.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.PaymentIntent.modify(
    ...
    def test_can_cancel_classmethod(self, request_mock):
        resource = stripe_sub5.PaymentIntent.cancel(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payment_intents/%s/cancel" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.PaymentIntent)
    ...
    def test_can_capture_classmethod(self, request_mock):
        resource = stripe_sub5.PaymentIntent.capture(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payment_intents/%s/capture" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.PaymentIntent)
    ...
    def test_can_confirm_classmethod(self, request_mock):
        resource = stripe_sub5.PaymentIntent.confirm(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payment_intents/%s/confirm" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.PaymentIntent)
