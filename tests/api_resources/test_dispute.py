...
class TestDispute(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.Dispute.list()
        request_mock.assert_requested("get", "/v1/disputes")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.Dispute)

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.Dispute.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/disputes/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Dispute)
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.Dispute.modify(
    ...
    def test_can_close_classmethod(self, request_mock):
        resource = stripe_sub5.Dispute.close(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/disputes/%s/close" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.Dispute)
