...
class TestTaxRate(object):
    def test_is_listable(self, request_mock):
        resources = stripe_sub5.TaxRate.list()
        request_mock.assert_requested("get", "/v1/tax_rates")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], stripe_sub5.TaxRate)

    def test_is_retrievable(self, request_mock):
        resource = stripe_sub5.TaxRate.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/tax_rates/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, stripe_sub5.TaxRate)

    def test_is_creatable(self, request_mock):
        resource = stripe_sub5.TaxRate.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = stripe_sub5.TaxRate.modify(
