...
class TestNewDefaultHttpClient(StripeClientTestCase):
    ...
    def setup_warnings(self, request_mocks):
        original_filters = stripe_sub5.http_client.warnings.filters[:]
        stripe_sub5.http_client.warnings.simplefilter("ignore")
        yield
        stripe_sub5.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(stripe_sub5.http_client, lib, None)

        inst = stripe_sub5.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), stripe_sub5.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), stripe_sub5.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), stripe_sub5.http_client.PycurlClient
        )
...
class TestRetrySleepTimeDefaultHttpClient(StripeClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = stripe_sub5.http_client.HTTPClient.MAX_DELAY
        stripe_sub5.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            stripe_sub5.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = stripe_sub5.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = stripe_sub5.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = stripe_sub5.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = stripe_sub5.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = stripe_sub5.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                stripe_sub5.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestHTTPClient(object):
    ...
    def setup_stripe(self):
        orig_attrs = {"enable_telemetry": stripe_sub5.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(stripe_sub5.http_client.HTTPClient):
            pass
...
class TestRequestsClient(StripeClientTestCase, ClientTestBase):
    REQUEST_CLIENT = stripe_sub5.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(stripe_sub5.error.APIConnectionError) as error:
...
class TestUrlFetchClient(StripeClientTestCase, ClientTestBase):
    REQUEST_CLIENT = stripe_sub5.http_client.UrlFetchClient
...
class TestUrllib2Client(StripeClientTestCase, ClientTestBase):
    REQUEST_CLIENT = stripe_sub5.http_client.Urllib2Client
...
class TestPycurlClient(StripeClientTestCase, ClientTestBase):
    REQUEST_CLIENT = stripe_sub5.http_client.PycurlClient
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            stripe_sub5.http_client.pycurl.error = FakeException
            mock.perform.side_effect = stripe_sub5.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(
            ...
            mock.setopt.assert_any_call(lib_mock.URL, stripe_sub5.util.utf8(url))
...
class TestAPIEncode(StripeClientTestCase):
    def test_encode_dict(self):
        ...
        values = [t for t in stripe_sub5.api_requestor._api_encode(body)]
